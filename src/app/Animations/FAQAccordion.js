...
export default class FAQAccordionElegant {
  ...
  _toggle(question, answer) {
    ...
    if (!this.allowMultiple && !isOpen) {
      ...
      this.questions.forEach((q) => {
        ...
        if (a !== answer) {
          GSAP.to(a, { height: 0, opacity: 0, y: -10, duration: 0.5, ease: "power2.inOut" });
          const icon = q.querySelector('.faq__icon');
          if (icon) GSAP.to(icon, { rotate: 0, duration: 0.5, ease: "power2.inOut" });
        }
      });
    }
    ...
    if (isOpen) {
      // close
      GSAP.to(answer, { height: 0, opacity: 0, y: -10, duration: 0.5, ease: "power2.inOut" });
    } else {
      // open elegantly
    ...
    if (icon) {
      GSAP.to(icon, { rotate: isOpen ? 0 : 45, duration: 0.5, ease: "power2.inOut" });
    }
  }
}
